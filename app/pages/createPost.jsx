...
function VoiceRecordButton({ recorder, onSend, theme }) {
...
function ChatScreenInner() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const [map, reads] = await Promise.all([
          fetchAllLastMessages(others, me),
          loadAllReadTimestamps(),
        ]);
        setLastMsgMap(map);
        setReadMap(reads);
      } catch (err) {
  ...
  const chatImageMessages = messages.filter((m) => m.type === MSG_TYPE.IMAGE && m.media_url);
  ...
  const renderUserRow = ({ item: u }) => {
    ...
    const mediaLabel = { image: "📷 Rasm", video: "🎥 Video", audio: "🎵 Audio", music: "🎵 Musiqa", voice: "🎤 Ovozli xabar", file: "📎 Fayl" }[lastMsg?.type];
    ...
    const lastTs = lastMsg?.timestamp || lastMsg?.created_at;
    const readTs = readMap[room];
    const isUnread = !!lastMsg && !isLastMine && (!readTs || (lastTs && new Date(lastTs) > new Date(readTs)));
  ...
  const renderMessageContent = (msg, isMe) => {
    switch (msg.type) {
      case MSG_TYPE.IMAGE:
        return (
          <TouchableOpacity onPress={() => openImageGallery(msg)} activeOpacity={0.9}>
      ...
      case MSG_TYPE.MUSIC:
        return <AudioMessageBubble uri={msg.media_url} duration={msg.duration} isMe={isMe} theme={t} label={msg.file_name || "Musiqa"} />;
      case MSG_TYPE.FILE:
        return (
          <FileMessageBubble
          ...
          {!activeUser ? (
            ...
            <>
              ...
              <View style={[styles.inputArea, { backgroundColor: t.surface, borderTopColor: t.border }]}>
                ...
                <View style={[styles.inputWrap, { backgroundColor: t.inputBg, borderColor: editingMsg ? t.accent : t.border, display: recorder.isRecording ? "none" : "flex" }]}>
                  ...
                  {input.trim() ? (
                    ...
                    <VoiceRecordButton recorder={recorder} onSend={handleVoiceSend} theme={t} />
                  )}
                </View>
              </View>
            </>
